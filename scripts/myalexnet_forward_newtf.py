...
for directory in directories:
	for filename in os.listdir(directory):
		...
		# print(filename)
		im1 = (imread(directory + "/" + filename)[:,:,:3]).astype(float32)
		im1 = im1 - mean(im1)
		im1[:, :, 0], im1[:, :, 2] = im1[:, :, 2], im1[:, :, 0]
		images.append(im1)
...
# Visualize using PCA (what's the effective difference between using PCA/TNSE? Which makes more sense for analysis here?)
pca = PCA(n_components=2)
# Need to reformat "images" list into a np array
#print(images)
#print(images[0][0,0,0])
X = np.empty((0, 227*227*3), float)
for image in images:
	flattened_image = np.reshape(image, (1, 227*227*3))
	#print(X.shape)
	#print(flattened_image.shape)
	X = np.append(X, flattened_image, axis=0)

X_proj = pca.fit_transform(X)
#Visualize as 2D scatter plot
carpet = plt.scatter(X_proj[:10,0], X_proj[:10, 1], c='r')
styrofoam = plt.scatter(X_proj[10:20,0], X_proj[10:20,1],c='b')
turf = plt.scatter(X_proj[20:30,0], X_proj[20:30,1], c='g')

plt.legend((carpet, styrofoam, turf), ("carpet images", "styrofoam images", "turf images"), loc='upper right')
