...
traj_paths=['straight','left', 'right', 'circle', 'zigzag']
counter = 0
fig = plt.figure()
fig.suptitle("Trajectory Following at Various Action Abstractions: Direct PWM (top), Velocity setpoints (bottom)", fontsize = 14, fontweight='bold')
...
while(counter<2):
	...
	for traj_path in traj_paths:
		...
		if(counter==0):
			save_dir='../run_0'+'/TRAJECTORIES'
		else:
			save_dir='../run_1'+'/TRAJECTORIES'


		plt.subplot(2,5, counter*5 + path_counter)
		...
		for j in range(5):
			...
			#calculate cost
			list_cost = []
			#print(counter, path_counter, actions_taken.shape)
			...
			cost_per_run_of_traj = np.sum(np.array(list_cost))
			cost_per_traj+= cost_per_run_of_traj/actions_taken.shape[0]

			plt.plot(traj_taken[:,0]-traj_taken[0,0],traj_taken[:,1]-traj_taken[0,1],'r')
		...
		plt.title("Cost: " + str(cost_per_traj/5))
		plt.plot(desired_states[0:limit,0]-desired_states[0,0],desired_states[0:limit,1]-desired_states[0,1],'k--', linewidth=4)
